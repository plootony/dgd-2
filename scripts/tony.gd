...
func _physics_process(delta: float) -> void:
	...
	if is_on_floor():
		...
		if not _idle and (_jump_time<=0 or (is_on_wall() and _jump_time<1)):
			...
			jumps_performed+=1
	else: velocity.y-=25*delta
	move_and_slide()
	net_grounded=is_on_floor()
	if position.y < -10:
		if Fusion.is_in_room(): replicator.teleport_3d(Vector3(4,1,7),Vector3.ZERO)
		else:
...
func _process(_delta: float) -> void:
	...
	if authority!=_authority:
		...
		physics_interpolation_mode=Node.PHYSICS_INTERPOLATION_MODE_ON if authority else Node.PHYSICS_INTERPOLATION_MODE_OFF
		reset_physics_interpolation()
