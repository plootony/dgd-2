...
func bake() -> void:
	var tony = "--tony" in OS.get_cmdline_user_args()
	var bone_prefix = "" if tony else "mixamorig1_"
	var skeleton_path = "Armature/Skeleton3D:" if tony else "Skeleton3D:"
	var source = load("res://Animation Library[Standard]/Godot/AnimationLibrary_Godot_Standard.glb").instantiate()
	var target = load("res://Tony/Tony.glb" if tony else "res://Bot/Bot.fbx").instantiate()
	...
	var names = {"DEF-hips":"Hips", "DEF-spine.001":"Spine", "DEF-spine.002":"Spine1", "DEF-spine.003":"Spine2", "DEF-neck":"Neck", "DEF-head":"Head"}
	for side in ["L", "R"]:
		var prefix = "Left" if side == "L" else "Right"
		for pair in [["shoulder","Shoulder"],["upper_arm","Arm"],["forearm","ForeArm"],["hand","Hand"],["thigh","UpLeg"],["shin","Leg"],["foot","Foot"],["toe","ToeBase"]]:
			names["DEF-"+pair[0]+"."+side] = prefix+pair[1]
		for pair in [["f_index","Index"],["f_middle","Middle"],["f_ring","Ring"],["f_pinky","Pinky"],["thumb","Thumb"]]:
			for i in range(1,4): names["DEF-%s.0%d.%s" % [pair[0],i,side]] = prefix+"Hand"+pair[1]+str(i)
	var mapping = {}
	for name in names:
	...
	var reference = []
	for i in src.get_bone_count(): reference.append(src.get_bone_global_pose(i))
	...
	for clip in ["Idle", "Walk", "Jog_Fwd", "Sprint", "Jump", "Jump_Start", "Jump_Land", "Crouch_Idle", "Crouch_Fwd"]:
		...
		animation.loop_mode = Animation.LOOP_NONE if clip.begins_with("Jump") else Animation.LOOP_LINEAR
		var tracks = {}
		for b in mapping:
		...
		for sample in range(int(ceil(original.length * 30)) + 1):
			...
			for b in dst.get_bone_count():
				...
				if mapping.has(b):
					var s: int = mapping[b]
					global_basis = src.get_bone_global_pose(s).basis * reference[s].basis.inverse() * dst.get_bone_global_rest(b).basis
					var rotation = (parent_basis.inverse() * global_basis).orthonormalized().get_rotation_quaternion()
			...
			hip_pos.y += (src.get_bone_global_pose(src_hips).origin.y - reference[src_hips].origin.y) * ratio
	...
	assert(ResourceSaver.save(library,"res://animations/tony_locomotion.res" if tony else "res://animations/bot_locomotion.res") == OK)
	source.free()
	target.free()
	quit()
