...
static func _curve(index: int) -> GradientTexture1D:
	if _curves.has(index):
		return _curves[index]
	# Per-channel curves: retain neutral highlights and a readable black level.
	var colors = [
		PackedColorArray(
			[Color(0, 0, 0), Color(0.25, 0.25, 0.25), Color(0.5, 0.5, 0.5), Color(1, 1, 1)]
		),
		PackedColorArray(
			[
				Color(0.016, 0.024, 0.021),
				Color(0.22, 0.245, 0.23),
				Color(0.48, 0.51, 0.48),
				Color(0.94, 0.96, 0.91)
			]
		),
		PackedColorArray(
			[
				Color(0.006, 0.015, 0.027),
				Color(0.19, 0.23, 0.28),
				Color(0.43, 0.49, 0.55),
				Color(0.94, 0.97, 1.0)
			]
		),
		PackedColorArray(
			[
				Color(0.004, 0.012, 0.022),
				Color(0.17, 0.23, 0.28),
				Color(0.39, 0.49, 0.55),
				Color(0.85, 0.96, 1.0)
			]
		)
	]
...
static func apply(
	env: Environment, sun: DirectionalLight3D, scene: Node, index: int, effects: bool
) -> void:
