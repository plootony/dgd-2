...
const ColorMoods = preload("res://ui/settings/color_moods.gd")
var config_path: String = CONFIG
const RESOLUTIONS = [
	Vector2i(1280, 720), Vector2i(1920, 1080), Vector2i(2560, 1440), Vector2i(3840, 2160)
]
var settings = {
	"preset": 0, "resolution": 2, "mode": 0, "vsync": true, "aa": 2, "post": true, "mood": 0
}
...
func _ready() -> void:
	...
	resolution = _option(
		box, "Разрешение", ["1280 × 720", "1920 × 1080", "2560 × 1440 (QHD / 2К)", "3840 × 2160"]
	)
...
func _describe(index: int) -> void:
	description.text = (
		(
			"Ultra: нативное QHD, тени 4096 / 4 каскада, мягкие тени, AO и непрямой свет высокого качества, отражения, ACES и мягкое свечение."
			if index == 0
			else "Минимальный: без теней и дорогих экранных эффектов; облегчённый рендер для нескольких клиентов."
		)
...
func _apply_requested() -> void:
	...
	settings = {
		...
		"mode": screen_mode.selected,
		"vsync": vsync.button_pressed,
		"aa": aa.selected,
		"post": post.button_pressed,
		"mood": mood.selected
	}
	apply_quality()
	var display_changed = (
		settings.resolution != _previous.resolution
		or settings.mode != _previous.mode
		or not _display_applied
	)
	if display_changed:
		apply_display()
...
func apply_quality() -> void:
	...
	environment.sky = (
		preload("res://environments/arena_sky.tres")
		if high
		else preload("res://environments/arena_sky_minimal.tres")
	)
	var viewport = get_tree().root
	viewport.msaa_3d = [
		Viewport.MSAA_DISABLED, Viewport.MSAA_DISABLED, Viewport.MSAA_4X, Viewport.MSAA_8X
	][settings.aa]
	viewport.use_taa = settings.aa >= 2
	viewport.screen_space_aa = (
		Viewport.SCREEN_SPACE_AA_FXAA if settings.aa == 1 else Viewport.SCREEN_SPACE_AA_DISABLED
	)
	...
	RenderingServer.directional_soft_shadow_filter_set_quality(
		RenderingServer.SHADOW_QUALITY_SOFT_ULTRA if high else RenderingServer.SHADOW_QUALITY_HARD
	)
	RenderingServer.environment_set_ssao_quality(
		RenderingServer.ENV_SSAO_QUALITY_ULTRA if high else RenderingServer.ENV_SSAO_QUALITY_LOW,
		not high,
		0.5,
		4 if high else 1,
		50,
	...
	RenderingServer.environment_set_ssil_quality(
		RenderingServer.ENV_SSIL_QUALITY_ULTRA if high else RenderingServer.ENV_SSIL_QUALITY_LOW,
		not high,
		0.5,
		4 if high else 1,
		50,
	...
	environment.tonemap_mode = (
		Environment.TONE_MAPPER_ACES if effects else Environment.TONE_MAPPER_LINEAR
	)
	...
	ColorMoods.apply(environment, sunlight, scene_root, settings.mood, effects)
...
func apply_display() -> void:
	...
	if settings.mode == 1:
		...
		var factor = minf(
			1, minf(float(usable.size.x - 40) / desired.x, float(usable.size.y - 80) / desired.y)
		)
...
func _node_added(node: Node) -> void:
	if node is MeshInstance3D:
		_filter_mesh.call_deferred(node)
...
func _save() -> void:
	...
	if error != OK:
		push_warning("Не удалось сохранить графические настройки: " + str(error))
