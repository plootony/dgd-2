...
const Graph = preload("res://ui/network/network_graph.gd")
...
func _ready() -> void:
	...
	_label(
		box,
		"↑ отправка — зелёный   ↓ приём — синий\nИстория 30 с · обновление 0.5 с\nТрафик клиента ↔ Photon Cloud\n1 КиБ = 1024 байта · общий поток",
		Color("97aabd")
	)
	_sample()
...
func _sample() -> void:
	...
	if not online:
		...
		_info.text = (
			"Shared · %s · ID %d%s · игроков %d/%d"
			% [
				Fusion.get_default_region(),
				Fusion.get_local_player_id(),
				" / Master" if Fusion.is_master_client() else "",
				room.get_player_count(),
				room.get_max_players()
			]
		)
		_latency.text = (
			"RTT: %.0f мс · сред. %.0f · макс. %.0f" % [latest_rtt_ms, average, _rtt.max()]
		)
		_traffic.text = (
			"↑ Отправка: %s\n↓ Приём: %s"
			% [_format_rate(latest_sent_bps), _format_rate(latest_received_bps)]
		)
		if latest_sent_bps >= 0:
			_push(_sent, latest_sent_bps / 1024)
		else:
			_sent.clear()
		if latest_received_bps >= 0:
			_push(_received, latest_received_bps / 1024)
		else:
			_received.clear()
